...
const getKeyAccessibleLabel = (keyCode: string): string =>
  KEY_ACCESSIBLE_LABELS[keyCode] ?? getKeyDisplayLabel(keyCode);

const KEY_INPUT_VALUES: Record<string, string> = {
  Backquote: "`",
  Minus: "-",
  Equal: "=",
  BracketLeft: "[",
...
const getKeyboardInputAction = (
  keyCode: string,
  keyValue?: string,
): KeyboardInputAction | null => {
  if (keyCode === "Backspace") return { type: "backspace" };
  if (keyCode === "Delete") return { type: "delete" };
  if (keyValue?.length === 1) return { type: "insert", value: keyValue };
  if (keyCode.startsWith("Key")) {
    return { type: "insert", value: keyCode.slice(3).toLowerCase() };
  }
  if (keyCode.startsWith("Digit")) {
    return { type: "insert", value: keyCode.slice(5) };
  }

  const value = KEY_INPUT_VALUES[keyCode];
  return value === undefined ? null : { type: "insert", value };
};
...
interface KeyboardContextType {
  ...
  typeKey: (keyCode: string, keyValue?: string) => void;
...
const KeyboardProvider = ({
  ...
  onType?: (action: KeyboardInputAction) => void;
}) => {
  ...
  const typeKey = useCallback(
    (keyCode: string, keyValue?: string) => {
      const action = getKeyboardInputAction(keyCode, keyValue);
      if (action) onType?.(action);
    },
    [onType],
  );
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const isKeyboardEditor =
        target instanceof HTMLElement && target.hasAttribute("data-keyboard-editor");
      const shouldType =
        !e.metaKey &&
        !e.ctrlKey &&
        !e.altKey &&
...
const KeystrokePreview = () => {
  const { lastPressedKey, pressedKeys, pressSequence } = useKeyboardSound();
  const displayKey =
    lastPressedKey &&
  ...
  return (
    ...
    >
      <div
        ...
        role="group"
        tabIndex={0}
        aria-label="Interactive mechanical keyboard. Press a key on your physical keyboard or click a key."
        className={cn(
