...
The desktop page is centered inside an 840 px maximum width with at least 20 px of inline breathing room. Sections are stacked vertically and separated primarily with whitespace; hairline rules reinforce major boundaries only.
...
The project overlay has a compact Details/System design/Chat tab row. Details follows a shared case-study structure across every project: the problem and constraints, the work owned, the concrete build, the outcome, and the technology stack. System design begins with the complete runtime map, then breaks the flow into smaller client, server, and backend views before documenting the decisions, trade-offs, recovery model, and trust boundary in readable language. Chat keeps the same dark surface as project details, with a quiet centered empty state, horizontally scrollable suggestion chips, repository-grounded responses, and a bottom-anchored composer. User messages receive a small elevated bubble; assistant answers remain visually open for long-form technical reading. Error and streaming states stay inline so the modal never jumps or becomes blocked.
...
Contact links use the registry-installed Aceternity floating dock, adapted only to the portfolio palette and destinations. The resting state is a compact chalk-neutral tray (`#f7f7f3`) with crisp ivory 40 px Hugeicon controls (`#fffefb`) and a cool neutral border (`#dbdad5`), keeping the dock clean against the warmer page. Pointer distance drives the source component’s `150 / 12 / 0.1` springs: nearby controls expand independently up to 80 px while their icons interpolate from 20 to 40 px, and the active label appears in a dark tooltip above the dock. The same four-control tray remains visible at narrow widths, giving touch users direct access to Email, LinkedIn, GitHub, and Wabi Sabi without an intermediate disclosure; pointer-only magnification simply stays dormant on touchscreens.
...
The desktop page adds a 56 × 320 px waveform scrubber at the trailing viewport edge without replacing the browser scrollbar. Thirty-seven deterministic bars form a quiet audio silhouette with no center spine, per-row guides, glow, or shadow; the exact scroll position controls the coral crest while only short-lived velocity energy passes through a damped spring. Hovering the rail, focusing its native vertical range, or dragging it reveals one 304px shadowless preview card rather than an index list. Its two-item footer shows only the current section number and name—progress remains available to assistive technology without becoming visual chrome. The inert, pointer-transparent 900 × 600 same-origin viewport is scaled to a 288 × 192 miniature and uses section anchors to follow the matching portion of the portfolio through Contact in the active theme. The preview runs on a dedicated no-index route with Lenis, the pull cord, Agentation, and recursive waveform rendering disabled. Coarse pointers and viewports below 900px neither render the control nor load the duplicate page; reduced motion removes energy and spatial transitions and updates the miniature only when the active section changes.
...
Motion is purposeful and occasional: cards provide press feedback, disclosure indicators communicate state, experience rows use a short opacity focus shift, technology pills reveal their labels over 300 ms, pill CTAs use a bounded magnetic spring, the waveform carries brief scroll energy, and the project modal explains the relationship between overview and detail. The technology-pill reveal intentionally mirrors its source component's measured CSS expansion; other UI transitions stay under 300 ms and use strong ease-out curves. Reduced-motion preferences remove the pill expansion, magnetic translation, waveform energy, synchronized miniature travel within a section, and spatial movement elsewhere while retaining brief opacity and color cues.

## Do's and Don'ts

The “What people say about me” section uses a soft chalk-white classroom board
